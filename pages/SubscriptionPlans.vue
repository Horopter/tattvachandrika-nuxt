<template>
  <div class="container mx-auto mt-8 px-4 relative">
    <!-- Loader Component: visible while any HTTP call is pending -->
    ...
    <nav aria-label="breadcrumb" class="mb-4">
      <ol class="flex space-x-2 text-gray-700">
        <li>
          <router-link to="/HomePage" class="text-blue-600 hover:text-blue-800">
            Home
          </router-link>
        </li>
        <li>
        ...
        <li class="text-gray-500 font-medium" aria-current="page">
          Subscription Plans
        </li>
      </ol>
    </nav>
            ...
            >
              ...
              <td class="px-4 py-4 text-sm text-gray-900">
                <template v-if="editPlanId === plan._id">
                  <input
                    type="date"
                    v-model="editPlanData.start_date"
              ...
              <td class="px-4 py-4 text-sm text-gray-900">
                <template v-if="editPlanId === plan._id">
                  <input
                    type="number"
                    v-model="editPlanData.subscription_price"
                    class="border border-gray-300 px-2 py-1 rounded-md w-full"
                  />
                </template>
                <template v-else>
              ...
              <td class="px-4 py-4 text-sm text-gray-900">
                <template v-if="editPlanId === plan._id">
                  <select
                    v-model="editPlanData.subscription_language"
                    class="border border-gray-300 px-2 py-1 rounded-md w-full"
                  >
                    <option
              ...
              <td class="px-4 py-4 text-sm text-gray-900">
                <template v-if="editPlanId === plan._id">
                  <select
                    v-model="editPlanData.subscription_mode"
                    class="border border-gray-300 px-2 py-1 rounded-md w-full"
                  >
                    <option
                      v-for="mode in modes"
              ...
              <td class="px-4 py-4 text-sm text-gray-900">
                <template v-if="editPlanId === plan._id">
                  <input
                    type="number"
                    v-model="editPlanData.duration_in_months"
                    class="border border-gray-300 px-2 py-1 rounded-md w-full"
                  />
                </template>
                <template v-else>
              ...
              <td class="px-4 py-4 text-sm flex space-x-2">
                <button
                  v-if="editPlanId === plan._id"
                  @click="savePlan(editPlanData)"
                  class="bg-green-600 text-white px-3 py-1 rounded-md text-xs shadow-sm hover:bg-green-700"
                >
                  Save
                </button>
                <button
                  v-if="editPlanId !== plan._id"
                  @click="editPlan(plan)"
                  class="bg-yellow-500 text-white px-3 py-1 rounded-md text-xs shadow-sm hover:bg-yellow-600"
                >
                  Edit
                </button>
                <button
                  ...
                  class="bg-gray-500 text-white px-3 py-1 rounded-md text-xs shadow-sm hover:bg-gray-600"
                >
                  Cancel
                </button>
                <button
                  ...
                  class="bg-red-600 text-white px-3 py-1 rounded-md text-xs shadow-sm hover:bg-red-700"
                >
                  Delete
                </button>
              </td>
...
export default {
  ...
  created() {
    // Trigger the individual HTTP calls.
    Promise.all([
      this.loadPlans(),
      this.loadLanguages(),
      this.loadModes(),
  ...
  methods: {
    loadPlans() {
      return this.runWithLoader(() => {
        return subscriptionPlanService
          ...
          .catch((error) => {
            console.error(
              "There was an error retrieving the subscription plans!",
              error
            );
          });
      });
    ...
    loadLanguages() {
      return this.runWithLoader(() => {
        return subscriptionPlanService
          ...
          .catch((error) => {
            console.error(
              "There was an error retrieving the languages!",
              error
            );
          });
      });
    ...
    addPlan() {
      const newPlan = {
        ...
        subscription_language:
          this.languages.length > 0 ? this.languages[0]._id : null,
        subscription_mode:
          this.modes.length > 0 ? this.modes[0]._id : null,
      ...
      return this.runWithLoader(() => {
        ...
        if (existingPlans.length > 0) {
          ...
          const highestVersion =
            validVersions.length > 0 ? Math.max(...validVersions) : 0;
        ...
        let promise;
        if (editedPlan._id) {
          promise = subscriptionPlanService.updatePlan(
            editedPlan._id,
            planData
          );
        } else {
          promise = subscriptionPlanService
            .createPlan(planData)
            .then(() => {
              this.isAddingNew = false;
            });
        ...
        return promise
          ...
          .catch((error) => {
            console.error(
              "There was an error updating/creating the subscription plan!",
              error
            );
          });
      });
    ...
    cancelEditPlan() {
      if (this.isAddingNew) {
        // For a new unsaved plan, remove it from the list.
        this.plans.shift();
        this.isAddingNew = false;
      } else {
        // For existing plans, clear the editing state.
        this.editPlanData = {};
      }
      this.editPlanId = null;
    },
      ...
      if (this.planToDelete) {
        return this.runWithLoader(() => {
          return subscriptionPlanService
            ...
            .catch((error) => {
              console.error(
                "There was an error deleting the subscription plan!",
                error
              );
            });
        });
